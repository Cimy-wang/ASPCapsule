...
class ASP_layer(Layer):
    def __init__(self, filters = 128, 
        ...
        super(ASP_layer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(ASP_layer, self).build(input_shape)
    ...
    def call(self, x):
        ...
        output = tf.nn.conv2d(map_all, tf.reshape(self.kernel, [1, 1, -1, self.filters]), strides = self.stride, dilations = self.dilation, padding = 'SAME')
        if self.use_bias:
            output += self.bias
        return output
    ...
    def get_config(self):
        ...
        base_config = super(ASP_layer, self).get_config()
